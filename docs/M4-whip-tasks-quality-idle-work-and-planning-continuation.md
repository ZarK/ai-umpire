...
Status: implemented by extending the trusted `planning` state model and shared config/decision/prompt/status/schema flow. Planning state now normalizes current phase, decisions, unresolved questions, draft paths, artifact status, provider status, concrete next actions, stop conditions, and supply-chain approval blocks. `planning.enabled: false` suppresses Bootstrap planning prompts. When enabled and idle, Umpire continues only a concrete trusted next planning action with a command, artifact check, or draft path. Human-blocking questions, ambiguous work-item mappings, active artifact inconsistency stop conditions, supply-chain approval blocks, malformed, stale, unknown, unsupported, and untrusted states stop instead of prompting. Planning prompts name the trusted source, command, artifact checks, draft paths, expected evidence, and prohibit invented product decisions, provider schema fields, work-item mappings, or acceptance criteria.
